...
class SparseMatrix(r: Int, c: Int, elements: Traversable[SparseMatrixNode])
    ...
    def this(rows: Int, columns: Int) = this(rows, columns, List[SparseMatrixNode]())
     ...
     */
    def set(row: Int, col: Int, value: Double): SparseMatrix = this remove(row, col) add(row, col, value)
     ...
     */
    def set(elems: Array[SparseMatrixNode]): SparseMatrix = (this /: elems) (
        (matrix: SparseMatrix, node: SparseMatrixNode) => node match {
    ...
    def positionInRange(row: Int, col: Int): Boolean = 
        MathUtils.valueInRange(row, 0, this.rows) && MathUtils.valueInRange(col, 0, this.columns)
     ...
     */
    private def get(row: Int, col: Int): Option[Double] = (Option(0D) /: (this map {
    ...
    private def remove(row: Int, col: Int): SparseMatrix = this rebuild (
        (this matrixList) filter ((node: SparseMatrixNode) => node match {
     ...
     */
    private def add(row: Int, col: Int, value: Double): SparseMatrix = this rebuild (
        (this matrixList) :+ SparseMatrixNode(row, col, value) )
}
